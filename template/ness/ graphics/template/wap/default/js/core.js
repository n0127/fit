var comic = {
	        ...
	        if(key != ''){
		        $.getJSON('//'+Mcpath.url+Mcpath.web+'index.php/api/data/comic?callback=?', {key:key}, function(res) {
		        	if(res.code == 1){
				        ...
				        if (a > 0) {
				            $(".hot-search").hide();
				            for (var o = 0; o < a; o++) c += '<div class="search-active-item" onclick="javascript:location.href=\''+ t[o].url+'\'" data-keyword="' + key + '" data-cid="' + t[o].id + '"><p class="comic-name">' + t[o].name.replace(n,
				            function(t) {
	...
	read : function(mid,cid,vip,cion){
	    ...
	    //离开前获取离开时间和已读图片数量
	    var ios = navigator.userAgent.match(/iPhone|iPod/i) != null;
	    if(ios){
