...
//长按事件
//long-press-event.min.js 搜索clearTimeout 增加了 _video.playbackRate===3?_video.playbackRate=1:''
...
function _check_mp4(url, callback) {
    var mp4boxfile = MP4Box.createFile(false);
    var check_ok = false;
    mp4boxfile.onReady = function (info) {
        check_ok = true;
        if (!('MediaSource' in window) || !MediaSource.isTypeSupported(info.mime)) {
            callback();
        }
    };
    var do_mp4_check = function (index) {
        if (check_ok || index >= 15) {
            return
        }
        send(url, function (data) {
            data.fileStart = (index - 1) * 1000000;
            mp4boxfile.appendBuffer(data);
            do_mp4_check(index + 1);

        }, undefined, undefined, {'Range': 'bytes=' + (index - 1) * 1000000 + '-' + (index * 1000000 - 1) + ''});
...
function _player(config) {
    ...
    if (mp) {
        playerConfig.parse = _loadType(url);
        mp.reloadUrl(url);
    ...
    mp.on('ready', function () {
        ...
        document.addEventListener('long-press', function (e) {
            var lock = document.getElementById("_lock");
            if (lock == undefined || lock.style.display == 'none') {
    ...
    console.error = function () {
        errorCount++;
        if (JSON.stringify(playerConfig.parse) == "{}") {
            repair()
        } else if (config.line == 'y') {
            repair('true');
        } else {
            if (errorCount >= (config.line != 'g' ? 8 : 4)) {
                repair();
                errorCount = 0;
            }
        }
...
function send(url, success, error, param, header) {
    try {
        var xhr = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTF");
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                if ((xhr.status >= 200 && xhr.status < 300) && ('response' in xhr ? xhr.response : xhr.responseText) != '' && success) {
                    success('response' in xhr ? xhr.response : xhr.responseText)
                } else if (error) {
                    error();
                }
            }
        ...
        if (param) {
            ...
            if (header) {
                for (var headerKey in header) {
                    if (headerKey.toLowerCase().indexOf("range") > -1) {
...
function sendDmk() {
    ...
    if (sendTime - dmkuTime < 1000 * 5) {
        mp.showToast("发送频率太快");
        return;
    }
    var time = Math.floor(_video.currentTime) * 1000;
    dmkuTime = sendTime;
    send('./dmku.php?vkey=' + _config['id'] + '&time=' + time + '&text=' + text + '&color=' + dmkuColor + '&size=' + dmkuFont, function () {
        danmaku.emit({
            text: text, style: {
...
function repair(param) {
    var config = _config;
    if (param) {
        config['fail'] = decodeURIComponent(param);
        player(config);
    } else {
        load_data(config);
    }
...
function load(arr, config, isJson) {
    var url = arr.shift();
    if (url == undefined || (!isJson && _loadTimeout == undefined)) {
        return
    }
    send(url, function (data) {
        var success;
        if (isJson) {
            try {
                ...
                load(arr, config, isJson);
                return;
            }
            url = url.split("url=")[1];
            success = data['code'] == 200 && data['url'].indexOf(url) == -1;
        } else {
            success = data.startsWith("#EXTM3U");
        }
        if (success) {
            config['url'] = isJson ? data['url'] : url;
            if (isJson && !(_data['s'].length == 0 || _data['b'].length == 0)) {
                send(data['url'], function () {
                    var j_div = document.createElement("div");
                    j_div.style.display = "none";
                    var j_body = document.getElementsByTagName("body")[0];
                    j_body.appendChild(j_div);
                    var j_mp = new MuiPlayer({
                        container: j_div, src: data['url'], muted: true, volume: 0, parse: _loadType(data['url'])
                    });
                    var j_destroy_callback = function (repeat) {
                        try {
                            j_mp.destroy();
                            j_body.removeChild(j_div);
                            if (repeat == false ? false : true) {
                                load(arr, config, isJson);
                            }
                        } catch (e) {
                        }
                    }
                    j_mp.on('error', j_destroy_callback);
                    j_mp.on('ready', function () {
                        var j_video = j_mp.video();
                        j_video.addEventListener("error", j_destroy_callback)
                        j_video.addEventListener("loadedmetadata", function () {
                            var compare;
                            try {
                                compare = _video.duration;
                            } catch (e) {
                            } finally {
                                if (isNaN(compare)) {
                                    compare = j_video.duration;
                                }
                            }
                            if (Math.abs(j_video.duration - compare) < 5 * 60) {
                                _player(config);
                            }
                            j_destroy_callback(false);
                    ...
                    load(arr, config, isJson);
                }, undefined, data['url'].indexOf(".mp4") > -1 ? {'Range': 'bytes=0-3000000'} : undefined)
                return
            }
            if (_loadTimeout != undefined) {
        ...
        if (url.indexOf(_domain) == -1) {
            arr.unshift(_domain + encodeURIComponent(url));
        }
        load(arr, config, isJson);
    }, undefined)
}
...
function load_data(config) {
    config['line'] = 'g';
    load(_data['g'], JSON.parse(JSON.stringify(config)), true);
    _loadTimeout = setTimeout(function () {
        config['line'] = 's';
        load(_data['s'], JSON.parse(JSON.stringify(config)), false);
        config['line'] = 'b';
        load(_data['b'], JSON.parse(JSON.stringify(config)), false);
    }, 0);
}
...
function player(config) {
    ...
    send("api.php", function (data) {
        var encrypt_key = CryptoJS.enc.Utf8.parse(config.id.substr(16))
        data = JSON.parse(CryptoJS.AES.decrypt(data, encrypt_key, {
            mode: CryptoJS.mode.CBC, iv: encrypt_key, padding: CryptoJS.pad.Pkcs7
        ...
        if (_data['y'].length != 0) {
            config['url'] = _data['y'][Math.floor(Math.random() * _data['y'].length)];
            config['line'] = 'y';
            _player(config);
            return
        }
